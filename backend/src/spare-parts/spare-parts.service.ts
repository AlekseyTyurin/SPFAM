...
export class SparePartsService {
  ...
  constructor(
    @InjectModel('SparePart') private readonly sparePartModel: Model<SparePart>,
  ) {}
  ...
  async getSearchSpareParts(searchQuery: string) {
    const spareParts = await this.sparePartModel.find({title: searchQuery}).exec();
    return spareParts.map((item) => ({
      id: item.id,
      title: item.title,
      description: item.description,
  ...
  private async findSparePart(title: string): Promise<any> {
    let sparePart;
    try {
      // sparePart = await this.sparePartModel.findById(id).exec();
      sparePart = await this.sparePartModel.find({ title: title }).exec();
    } catch (error) {
